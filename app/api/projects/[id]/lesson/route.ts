...
export const GET = handler(async (req: Request, ctx: { params: Promise<{ id: string }> }) => {
  ...
  if (!draft) {
    return fail(404, "Project not found. It may have expired — start a new project.");
  }
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.error(`[lesson] AI generation failed id=${id} world=${draft.worldSlug}: ${message}`);
    ...
    return fail(502, `AI lesson generator unavailable: ${message.slice(0, 200)}`);
  }
});
