...
export type InvokeTeacherAgentParams = {
  agentName: string;
  /**
   * Parameters passed to ContextForge A2A invocation.
   * For jsonrpc agents these are forwarded as the JSON-RPC params.
   */
...
/**
 * Invoke an A2A agent via ContextForge.
 * Endpoint: POST /a2a/{agent_name}/invoke
 */
export async function invokeTeacherAgent({
  agentName,
  parameters,
  interactionType,
}: InvokeTeacherAgentParams): Promise<any> {
  if (!CF_BASE) {
